...
\end{align}
First observe that any three of these equations together forms a singular linear system.
\\\\
...
\end{align}
This reveals two facts. The first fact is that the only relevant equations are 34-36. The second is that 37 implies that
\begin{align}
0&=k^2\ps{\f{\Sigma_{yz}}{k_yk_z}+\f{\Sigma_{zx}}{k_zk_x}+\f{\Sigma_{xy}}{k_xk_y}}\,,
\end{align}
i.e.
\begin{align}
0&=k_x\Sigma_{yz}+k_y\Sigma_{zx}+k_z\Sigma_{xy}\,.
\end{align}
That is to say $(\Sigma_{yz},\Sigma_{zx},\Sigma_{xy})$ is transverse. Because the only three relevant components are those of this vector, denote $\Sigma=(\Sigma_{yz},\Sigma_{zx},\Sigma_{xy}):=(\Sigma_x,\Sigma_y,\Sigma_z)$.
\\\\
We now solve for $A_i$ in terms of $\Sigma_i$.
\\\\
