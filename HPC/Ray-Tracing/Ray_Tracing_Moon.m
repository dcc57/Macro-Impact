...
vX = 2.5 * 10^5; %m/s
sigmaX = (10^(-7)) * (10^(-4));% m^2
fhigh = 20;%1/s %HIGHEST DETECTABLE FREQUENCY
TotalDensityAverage = 3.3*10^3; %THE AVERAGE DENSITY THROUGHOUT THE WHOLE MOON
pbar = 10^8;%HOOKIAN PRESSURE kg/(m*s^2)
vs = 4.4 * 10^3;%AVERAGE SHEAR WAVE VELOCITY THROUGHOUT THE WHOLE MOON m/s
vp = 7.6 * 10^3;%AVERAGE PRESSURE WAVE VELOCITY THROUGHOUT THE WHOLE MOON m/s
...
AverageV = [8.15502,7.96714,7.81709,7.65888,7.56174,5.5,3.2,1.0].*10^3; %THE AVERAGE VELOCITY IN EACH OF THESE REGIONS km/s
...
e = 70.*10^3; %NOTE THE DIFFERENCE IN UNITS! m
...
%FRACTION OF ENERGY IN LOWER SPECTRUM

mu = (vs^2)*TotalDensityAverage;
lambda = ((vp^2) - 2*(vs)^2) * TotalDensityAverage;
BulkMod = lambda + (2/3)*mu;
k1 = fhigh/(2 * pi * vp);
kappa = (BulkMod^2)/(lambda + 2 * mu);
epsilon = TotalDensityAverage * sigmaX * vX^2;

Xi = (9 * kappa * epsilon * k1^2)/(16 * pi * pbar^2);
...
outputmat = cat(2,Xfinal,Yfinal,Zfinal,OutTim,Density,REF,ATT);
outputnum = [M,N,L,D];

save PointsTestMat.mat outputmat;
save PointsTestNum.mat outputnum;