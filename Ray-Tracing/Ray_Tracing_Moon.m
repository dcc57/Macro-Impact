...
%%{
%VPREMOON MODEL
a = [8.26955,8.26955,8.26955,8.26955,8.26955,5.5,3.2,1.0].^(1/2); %THESE ARE THE CONSTANT TERMS IN THE PARABOLIC VELOCITY FIELD STRUCTURE, THE FIRST ENTRY CORRESPONDING TO THE CORE km/s
b = [2.47697*10^(-7),2.47697*10^(-7),2.47697*10^(-7),2.47697*10^(-7),2.47697*10^(-7),10^(-11),10^(-11),10^(-11)].^(1/2); %THESE ARE THE COEFFICIENTS OF THE QUADRATIC TERM IN THE VELOCITY FIELD 1/(s*km)

Radii = [0,961.7,1231.7,1461.7,1671.7,1709.1,1725.1,1736.1,1737.1]; %NOTE THAT WE MUST TAKE 0 AS A LAYER FOR TECHNICAL PURPOSES km

Q = [675,1125,3375,9000,6750,6750,6750,6750]; %THE QUALITY FACTOR IN THE DIFFERENT REGIONS

AverageV = [8.15502,7.96714,7.81709,7.65888,7.56174,5.5,3.2,1.0]; %THE AVERAGE VELOCITY IN EACH OF THESE REGIONS km/s
...
Xi = (9/4)*(BulkMod^2)*(pi^2)*TotalDensityAverage*sigmaX*(vX^2)/((lambda + 2*mu)*(pbar*lambda1)^2);
