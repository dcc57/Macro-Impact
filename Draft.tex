...
\author{David Cyncynates}
\affiliation{Physics Department\\ Case Western Reserve University\\ Cleveland, Ohio 44106-7079, USA}

\begin{abstract}
Recent limits have been proposed constraining the available parameter space of macroscopic dark matter (Macros). These limits were based on the energy deposition of Macros into low frequency seismic modes being akin to that of a nuclear or chemical bomb. We show that this assumption does not hold by considering the mechanism by which the Macro impact generates seismic waves. Our new estimates take into account the effect of seismometer sensitivity and energy distribution across frequencies, anelastic attenuation, geometric effects, internal anisotropy of the Earth or Moon, as well as energy loss due to melting. We produce limits on the parameter space of Macros which can be ruled out by seismic event rate.
\end{abstract}
\maketitle
...
\end{align}
where $f$ is an analytic function whose range is a subset of $[-1,1]$, and $p_0$ is the maximum differential pressure in this region. After a short time, the pressure is still of this form, since the anisotropy of the Moon is significant only on large scales.
...
\section{Conclusion}
For Macros with mass to area ratio sufficient to pass nearly unimpeded through the Moon, and in particular nuclear dense objects such as strange quark nuggets, we have demonstrated that seismological constraints are insufficient to rule them out. In order to use the Moon as a low noise detector for high density Macros, it would be necessary to increase the usable frequency range of seismometers to several hundred Hertz. However, since the Moon (and Earth) preferentially attenuate higher frequency modes, it is unlikely that measuring high frequencies would greatly improve sensitivity to these types of impacts.

On the other hand, increasing the lunar seismometer sensitivity to small displacements would increase the volume of Macro parameters one could rule out. That said, it is uncertain whether seismic noise at such small displacements would make Macro impacts indistinguishable from the background.
