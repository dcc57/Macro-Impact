...
\end{align}
Notice that this has the form of $\sqrt{W(x)\ln\p{{\frac{x}{W(x)}}}}$, which gives $\sqrt{W(x)^2}$ by the identity $\ln\p{{\frac{x}{W(x)}}} = W(x)$ which holds when $x \geq -1/e$r which is true for the parameters at hand.  Thus, we have,
\begin{align}
<<<<<<< HEAD
