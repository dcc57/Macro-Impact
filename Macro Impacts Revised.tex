...
\\\\
To keep track of the quality factor, we need to be able to pointwise multiply $e^{-\f{k}{2Q}t}$ by $A(k)$. The problem is that Matlab doesn't really do functions... Well... Suppose we start with an amplitude $A(0,k)$, and that the ray propagates through a region of quality factor $Q_1$ for time $\Delta t_1$. Then the amplitude after the propagation is $A(\Delta t_1, k)=A(0,k)F_1 e^{-\f{k}{2Q_1}\Delta t_1}$, where $F_1$ is a transmission or reflection coefficient, which is not a function of $k$. Now, suppose it propagates through another region with quality factor $Q_2$ for $\Delta t_2$. Then we have $A(\Delta t_1 + \Delta t_2, k)=A(0,k)F_1F_2 e^{-\f{k}{2}\p{\f{\Delta t_1}{Q_1}+\f{\Delta t_2}{Q_2}}}$. That's straight forward enough. Therefore, to understand what happens to a general ray's amplitude, we only need to know two things:
\begin{align}
F &= \prod_i F_i\,,\\
\f{T}{Q} &= \sum_i\f{\Delta t_i}{Q_i}\,.
\end{align}
...
\begin{align}
p_0(m)^2 = \f{ K_m^2\rho_m}{\lambda_m + 2\mu_m}\f{2l^2\sigma_Xv_X^2}{\int r\diff r f(r)^2}\,.
\end{align}
...
\notag &=\f{Q}{NT^2}F_{m,n}^2e^{-\f{k_0^2T}{Q}}\ps{4T(1-\cos(l k_0)) + \I e^{\f{k_0 T}{Q}} l Q\p{2\te{Ei}\ps{-\I l \notag k_0-\f{k_0 T}{Q}} - 2\te{Ei}\ps{\I l k_0 - \f{k_0 T}{Q}} + \right.\right.\\&\left.\left.+\ln\ps{-\f{Q}{\I l Q+T}}+\ln\notag \ps{-\f{\I l Q + T}{Q}} + 2\ln\ps{\I l - \f{T}{Q}}} - 4 l(Q + k_0 T)\te{Si}\ps{l k_0}}\times\\
&\times \underbrace{\f{(b^2f_1(b)-a^2f_1(a))^2}{\int r\diff r f(r)^2}}_{A}\rho_m2l^2\sigma_Xv_X^2\,.
\end{align}
...
\begin{align}
E_m'\leq F_{m,n}^24 l p_0^2 \f{Q^2}{T^2} \arctan\ps{l\f{Q}{T}} A\rho_m2l^2\sigma_Xv_X^2\,.
\end{align}
...
\\\\
Now, for a ray that immediately hits the surface, its energy must be the same as when it started. This corresponds to $T = \to 0$, and
\begin{align}
E_m = 4l E_mAF_{m,n}^2\p{-2k_0+k_0\cos(h k_0)+\f{\sin(h k_0)}{h}+h k_0^2\te{Si}(h k_0)}
\end{align}
where we've taken $l\rho_m\sigma_Xv_X^2=E_m$.
\begin{align}
 \p{4lF_{m,n}^2\p{-2k_0+k_0\cos(h k_0)+\f{\sin(h k_0)}{h}+h k_0^2\te{Si}(h k_0)}}^{-1}=  A
\end{align}
These units are way wrong. Gonna fix tomorrow.
\pagebreak
