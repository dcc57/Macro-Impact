...
&=\ps{\f{4\pi p_0}{k\cos\theta}\sin\p{\f{h k\cos\theta}{2}}x^2\underbrace{\sum_{n=0}^\infty\sum_{m=0}^n\f{f^{(n)}(r_0)}{n!}\f{(-r_0)^{n-m}}{2+m}x^{m}}_{f_1(x)}}_{x=a}^b+\scr O(k)\,,\\
...
\notag\theta(s)&=\arccot\ps{\f{4(-1)^{s_2}abx_0(a^2-b^2x_0^2)\sin\alpha}{4a^2b^2(1+e^{4abs})x_0^2+4(-1)^{s_2}abe^{4abs}x_0(a^2+b^2x_0^2)\cos\alpha+(e^{4abs}-1)(a^4+b^4x_0^4+2a^2b^2x_0^2\cos(2\alpha))}}-\\
