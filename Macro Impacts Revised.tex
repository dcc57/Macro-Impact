...
\begin{align}
\mat{c}{a'\\b'\\c'}=\mat{ccc}{\cos\alpha&0&\sin\alpha\\0&1&0\\-\sin\alpha&0&\cos\alpha}\mat{c}{a\\b\\c}
\end{align}
...
\begin{align}
\alpha=\arctan\p{\f{b''}{a''}}\,.
\end{align}
We can now use the work from above. 
\item CHECK: Is $x_0>R_{\te{mantle}}$?
\begin{enumerate}
\item If yes, then CHECK: is $s_{\te{mantle}}$ real?
\begin{enumerate}
\item If yes, then compute $\theta(s_{\te{mantle}})$ and $\sin\phi(s_{\te{mantle}})$ from the formulas in the Numerical Methods section. We now have a new starting angle $\alpha=-\arcsin\phi(s_{\te{mantle}})$. This information is then saved along side the angles $\alpha$ and $\beta$ to be used in the next round of computation.
...
\item At this point, we have computed final angles for all rays in their respective frames of reference. What is left is to rotate these points back into the original frame. Since all of these coordinates have the same radius, it is only important to keep track of the angle.
\end{enumerate}
\pagebreak
\section{Melt Zone}
