...
\end{align}
It is then obvious that $b=0$ corresponds to straight lines. The importance of this observation is that $b\approx 10^{-7}\te{s}^{-1}$ and $a\approx 10\te{km s}^{-1}$, so the radius of the Poincare sphere corresponding to the trajectories of $p$-waves in earth is approximately $10^8\te{km}$, while the earth is of radius $10^3\te{km}$. Hence, the smallest radius of curvature of an arc will be roughly $10^{-5}\te{km}$. Thus, it is a reasonable first approximation to take the seismic waves to propagate in straight lines.
\pagebreak

We now want to calculate the distribution of rays about the perimeter of a circle. In essence, we want to know $\diff\theta(R_{\te{earth}})$ as a function of $p$ and $\theta_0$. First we pick a point $p$ and direction $\psi$. we now parametrize all ray trajectories passing through this point by $\psi$. We now fix a $\psi$ and add to it $\delta\psi$, chosen small enough so that the two corresponding trajectories are close together. We then calculate $\delta\theta(R_{\te{earth}})$, the angular distance between the two rays as measured from the origin when they are at the radius of the earth.
\\\\
TO DO: Parametrize the rays through a point. Get the final parameter $s(R_{\te{earth}})$ as a function of the angle through the point. Then figure out how a small change in the initial angle changes the final angle. Get an approximate model for the velocity field through the earth.
...
%\pagebreak
%Meeting Agenda: Found bounds for $\Xi$. We need to map intensity to the surface - how? Do we make a generous over estimate again? If so, how should we approach it?
\end{document}
